...
predict_gene_expression <- function(genelist, dosages, prefix, weights, output) {
  ...
  all_applications <- data.table(
    dbGetQuery(weights.db.connection,
               "SELECT rsid, gene, weight, eff_allele FROM weights ORDER BY rsid"))
  setkey(all_applications, rsid)
  all_applications$id <- 1:nrow(all_applications)
  ...
  while(!any(is.na(ret <- get_next_dosage()))){   
   all_applications[ret$rsid, transcription_matrix_update(genelist, gene, weight, eff_allele, ret$refallele, ret$dosages), by=id]
   }
