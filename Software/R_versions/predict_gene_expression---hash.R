...
predict_gene_expression_old <- function(genelist, dosages, prefix, weights, output) {
  weights.db.connection <<- dbConnect(RSQLite::SQLite(), dbname=weights)
  print(paste(Sys.time(), "Preloading weights..."))
  all_applications <- hash()  
  ddply(
    dbGetQuery(weights.db.connection,
               "SELECT rsid, gene, weight, eff_allele FROM weights ORDER BY rsid"),
    .(rsid),
    function (ds) {all_applications[ds$rsid[1]] <- as.data.frame(ds)})
  
  print(paste(Sys.time(), "Preloading done", nrow(all_applications)))
  print(paste("checking", "rs139202"))
  print(all_applications[["rs139202"]])
  print("----")
  
  get_next_dosage <- get_all_dosages(dosages, prefix)    
  while(!any(is.na(ret <- get_next_dosage()))){   
  if (has.key(ret$rsid, all_applications)) {
   ds <- all_applications[[ret$rsid]]
   genes <- ds$gene
   weights <- ds$weight
   eff_alleles <- ds$eff_allele
   
   for (i in 1:nrow(ds)){
     transcription_matrix_update(genelist, genes[i], weights[i], eff_alleles[i],
                                 ret$refallele, ret$dosages)   
   }}}
...
predict_gene_expression <- function(genelist, dosages, prefix, weights, output) {
  ####
  print(paste(Sys.time(), "Building gene x rsid[int] -> weight, eff_allele matrices"))
  weights.db.connection <<- dbConnect(RSQLite::SQLite(), dbname=weights)
  
  # This would be faster if we already had pre-coded integers for rsids, but it's O(1).  
  df <- dbGetQuery(weights.db.connection,
             "SELECT rsid, gene, weight FROM weights ORDER BY rsid")
  weight.matrix <- as.matrix(reshape(df, direction="wide", timevar="rsid", idvar="gene"))
  x <- colnames(weight.matrix)
  rsid_int <- hash(x[2:length(x)], 1:(length(x)-1))
