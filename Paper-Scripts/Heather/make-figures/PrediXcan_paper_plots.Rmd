...
dataL <- alpha0.5h2 %>% mutate(ymin = pmax(0, h2 - 2 * se), ymax = pmin(1, h2 + 2 * se)) %>% arrange(h2) %>% select(gene,R2,h2,ymin,ymax)
...
a<-inner_join(dataL,dataP,by='gene')
b<-inner_join(a,dataT,by='gene')
