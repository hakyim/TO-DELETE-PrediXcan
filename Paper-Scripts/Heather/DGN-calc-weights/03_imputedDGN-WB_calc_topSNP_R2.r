...
resultsarray <- array(NA,c(length(explist),4))
dimnames(resultsarray)[[1]] <- explist
resultscol <- c("gene","top.snp","R2","pval")
dimnames(resultsarray)[[2]] <- resultscol
workingbest <- "working_" %&% tis %&% "_exp_topSNP_R2_imputedSNPs_chr" %&% chrom %&% "_" %&% date %&% ".txt"
write(resultscol,file=workingbest,ncolumns=4,sep="\t")

weightcol = c("gene","SNP","refAllele","effectAllele","beta")
workingweight <- en.dir %&% tis %&% "_polyscore_topSNP_weights_chr" %&% chrom %&% "_" %&% date %&% ".txt"
...
for(i in 1:length(explist)){
  ...
  if(is.null(dim(cisgenos))){
    ...
    resinfo <- c(gene,"no.cis.snps",NA,NA)
	...
	topres <- betares[order(betares[,2]),]
	topsnp <- rownames(topres)[1] ###pull top snp with lowest P
        betas <- topres[1,1]
        names(betas)<-topsnp
	...
	### polyscore
        ...
        if(is.array(testgenos)=='FALSE'){ ### calcs polyscore if only one individual in test set, happens when expression sample size is low (<100)
          ...
          }else{ ### calcs polyscore if only one beta, which is what we have here
      ...
      alltopres <- allbetares[order(allbetares[,2]),]
      alltopsnp <- rownames(alltopres)[1] ###pull top snp with lowest P
      allbetas <- alltopres[1,1] ###pull top beta
      names(allbetas) <- alltopsnp
      allbetalist <- names(allbetas)
      allbetainfo <- bim[allbetalist,]
      betatable <- as.matrix(cbind(allbetainfo,alltopres[1,1],alltopres[1,2]))
      ...
      resinfo <- c(gene,alltopsnp,res$r.squared,res$coef[2,4])
      resultsarray[gene,] <- resinfo  
    }
  }
...
write.table(resultsarray,file=tis %&% "_exp_" %&% k %&% "-foldCV_polyscore_topSNP_imputedSNPs_chr" %&% chrom %&% "_" %&% date %&% ".txt",quote=F,row.names=F,sep="\t")
